...
const RELATED_DOCUMENTS = [
  {
    id: "doc-5",
    title: "Sổ tay tóm tắt trọn bộ công thức Vật lý đại cương 1",
    school: "HUST",
    downloads: 3560,
    rating: 5.0,
    credits: 1,
  ...
  {
    id: "doc-9",
    title: "Đề cương ôn thi cấp tốc Đại số tuyến tính",
    school: "HUST",
    downloads: 4120,
    rating: 4.8,
    credits: 1,
  ...
  {
    id: "doc-13",
    title: "Ngân hàng đề thi trắc nghiệm Tin học đại cương HUST",
    school: "HUST",
    downloads: 1980,
    rating: 4.7,
    credits: 2,
  ...
  {
    id: "doc-14",
    title: "Giáo trình Giải tích 1 - Tác giả Nguyễn Đình Trí",
    school: "HUST",
    downloads: 2890,
    tracking: 4.9,
    credits: 3,
...
export default function DocumentDetailPage() {
  ...
  useEffect(() => {
    const loadDocument = async () => {
      // Chặn an toàn: Nếu id bị trống hoặc là chuỗi "undefined" do lấy sai thuộc tính từ trang trước
      if (!id || id === "undefined") {
      ...
      try {
        setIsLoading(true);
        // Sửa tại đây: Chỉ truyền duy nhất biến id động lấy từ useParams()
  ...
  const handleDownload = () => {
    ...
    alert(
      `Hệ thống đang trừ ${documentData.creditCost ?? 0} lượt tải để bắt đầu tải tệp tin: ${documentData.size || ""}`,
    );
  };
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      ...
      const response = await documentApi.submitComment(
        documentData.id,
        newComment,
      );
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans antialiased selection:bg-indigo-500 selection:text-white">
      ...
      <div className="bg-white border-b border-slate-200 py-2.5 px-4">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-xs font-bold text-slate-400 overflow-x-auto whitespace-nowrap">
          ...
          {/* school -> universityId */}
          <span className="text-slate-600 hover:text-indigo-600 cursor-pointer">
            Tài liệu {documentData.universityId}
          </span>
          <ChevronRight className="w-3.5 h-3.5" />
          {/* subject -> subjectName */}
          ...
          <div className="space-y-6">
              ...
              <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-3 rounded-xl text-xs">
                <div>
                  ...
                  <div className="text-base font-extrabold tracking-tight text-emerald-400">
                    -{documentData.creditCost ?? 0} Lượt
                  </div>
                </div>
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
              <h3 className="font-extrabold text-slate-900 tracking-tight text-xs md:text-sm uppercase text-indigo-600 border-b border-slate-100 pb-2 flex items-center gap-1.5">
                <MessageSquare className="w-4 h-4" /> Thảo luận sinh viên (
                {comments.length})
              </h3>
              ...
              <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1">
                {comments.map((comment) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <img
                          ...
                          alt={comment.user || comment.userName}
                          className="w-5 h-5 rounded-full object-cover"
                        />
                        <span className="font-bold text-slate-800">
                          @{comment.user || comment.userName || "sinhvien"}
                        </span>
                      </div>
                    ...
                    <p className="text-slate-600 pl-6 font-normal leading-normal">
                      {comment.content}
                    </p>
                  </div>
                ))}
