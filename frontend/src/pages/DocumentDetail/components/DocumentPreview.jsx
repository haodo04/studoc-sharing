...
export default function DocumentPreview({ 
  ...
  const handleOpenInNewTab = () => {
    const documentUrl = documentData?.fileUrl || documentData?.previewUrls?.[0];
    if (documentUrl) {
      window.open(documentUrl, "_blank", "noopener,noreferrer");
    } else {
      alert("Không tìm thấy đường dẫn tài liệu gốc để xem toàn bộ.");
    }
  };
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
      ...
      <div className="bg-slate-100 flex-1 max-h-[650px] overflow-y-auto relative flex flex-col items-center custom-scrollbar">
        ...
        {documentData?.previewUrls && documentData.previewUrls.length > 0 ? (
          <div className={`p-4 md:p-6 space-y-4 w-full flex flex-col items-center ${!hasUnlockedFull ? 'pb-44' : 'pb-24'}`}>
            {(hasUnlockedFull ? documentData.previewUrls : documentData.previewUrls.slice(0, 2)).map((url, index) => (
              <img
                key={index}
                src={url}
          ...
          <div className="absolute inset-x-0 bottom-0 pt-36 pb-6 bg-gradient-to-t from-slate-950 via-slate-900/95 to-transparent z-20 flex flex-col items-center justify-end p-6 text-center text-white">
            <div className="max-w-md space-y-3 pb-2 w-full">
              ...
              <p className="text-[11px] text-slate-400 px-6 leading-normal">
                Sử dụng <span className="text-indigo-400 font-bold">{documentData?.creditCost || 0} Xu</span> để mở khóa quyền sở hữu. Bạn sẽ không bị che mờ nội dung và được tải file gốc bất kỳ lúc nào.
              </p>
              ...
              >
                <Unlock className="w-4 h-4" /> Mở khóa ngay ({documentData?.creditCost || 0} Xu)
              </button>
            </div>
          </div>
        )}
        ...
        {hasUnlockedFull && (
          <div className="absolute inset-x-0 bottom-0 bg-slate-900/90 backdrop-blur-sm border-t border-slate-800 p-4 z-20 flex flex-col sm:flex-row items-center justify-between gap-3 text-white">
            <div className="text-center sm:text-left">
              ...
              <p className="text-[10px] text-slate-400">Bạn được chọn các hành động hoàn toàn miễn phí dưới đây.</p>
            </div>
