...
const PublicFileView = () => {
  ...
  useEffect(() => {
    const getFile = async () => {
      setIsLoading(true);
      try {
        // Re-add token fetching and authorization header
        const res = await axios.get(apiEndpoints.PUBLIC_VIEW_FILE(fileId));
        setFile(res.data);
        setError(null);
      } catch (error) {
  ...
  const handleDownload = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute("download", file.name); // Use the actual file nam
